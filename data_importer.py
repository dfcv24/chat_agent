...
class DataImporter:
    ...
    async def import_json_file_large(self, file_path: str, table_name: str, 
                                   drop_if_exists: bool = False, batch_size: int = 1000) -> Dict[str, Any]:
        """
        导入大型JSON文件到PostgreSQL（流式处理）
        ...
        Returns:
            导入结果统计
        """
        logger.info(f"开始流式导入大型JSON文件: {file_path}")
        ...
        for obj in self.stream_json_objects(file_path):
        ...
        if not sample_data:
            raise ValueError("JSON文件为空或无法解析")
        
        # 根据样本数据创建表
        create_sql, column_mapping = await self.create_table_from_sample(
            table_name, sample_data, drop_if_exists
        )
        ...
        async with self.connection_pool.acquire() as conn:
            ...
            for obj in self.stream_json_objects(file_path):
                ...
                if len(batch_data) >= batch_size:
                    ...
                    logger.info(f"已插入 {inserted_rows} 行")
                    batch_data.clear()
                    gc.collect()  # 手动垃圾回收
        ...
        result = {
            ...
            "processing_mode": "large_file_streaming",
            "status": "success"
        }
        
        logger.info(f"大型JSON文件导入完成: {inserted_rows} 行数据")
        return result
